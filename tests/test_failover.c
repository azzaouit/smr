...
void *client_thread(void *p) {
  ...
  if (n.c->host_id == 0) {
    ...
    if (n.c->host_id == 1) {
      buf = calloc(1, SMR_MAX_BUF);
      assert(buf);
      for (int i = SS; i < SMR_MAX_SLOTS; ++i) {
        for (int j = 0; j < SMR_MAX_BUF; ++j)
          buf[j] = i;
        assert(!consensus_propose(&n, buf, SMR_MAX_BUF));
      }
  ...
  assert(h->size == (n.c->host_id ? SMR_LOG_SIZE : SMR_LOG_SIZE >> 1));
  assert(h->capacity == SMR_LOG_SIZE);
  for (uint32_t i = 0; i < (n.c->host_id ? SMR_MAX_SLOTS : SS); ++i) {
