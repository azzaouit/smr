...
/* Number of 8-byte CAS operations needed */
#define MEM_TX_N_QWORDS(t) ((t->len + 7) >> 3)
...
int rdma_cas(struct rdma *r, struct mem_tx *t, uint16_t id) {
    ...
    size_t num_ops = MEM_TX_N_QWORDS(t);
    uint64_t ra =
        (t->remote_addr ? t->remote_addr : a->addr) + t->remote_offset;

    if (num_ops == 0) return -EINVAL;

    struct ibv_send_wr wrs[num_ops], *bad_wr = NULL;
    struct ibv_sge sges[num_ops] = {};

    // Build chain of CAS work requests
    for (size_t i = 0; i < num_ops; ++i) {
        uint64_t offset = i << 3;

        // Setup SGE for this operation
        sges[i].addr = t->local_addr + offset;
        sges[i].length = 8;  // fixed 8-bytes for CAS operations
        sges[i].lkey = r->mr[t->local_plane]->lkey;

        // Setup work request
        memset(&wrs[i], 0, sizeof(struct ibv_send_wr));
        wrs[i].wr_id = RDMA_SEND_WRID;
        wrs[i].sg_list = &sges[i];
        wrs[i].num_sge = 1;
        wrs[i].opcode = IBV_WR_ATOMIC_CMP_AND_SWP;

        // Only signal on last operation
        if (i == num_ops - 1) {
        ...
        // Get compare and swap values for this 8-byte chunk
        uint64_t cmp = ((uint64_t *)(t->compare_addr))[i];
        uint64_t swp = ((uint64_t *)(t->swap_addr))[i];

        // Setup atomic parameters
        wrs[i].wr.atomic.remote_addr = ra + offset;
        wrs[i].wr.atomic.rkey = a->rkey;
        wrs[i].wr.atomic.compare_add = cmp;
        wrs[i].wr.atomic.swap = swp;
    }

    // Post the chain of work requests
    return ibv_post_send(q, wrs, &bad_wr);
}
#endif
...
int rdma_set_cpu_affinity(struct rdma *r) {
    ...
    FILE *f = fopen(path, "r");
    if (f != NULL) {
        if (fscanf(f, "%d", &r->cpu_affinity) != 1) {
