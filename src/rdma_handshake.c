  ...
  for (size_t i = 0; i < expected_clients; ++i) {
    ...
    if (clientfd < 0)
      ...
      for (int plane = 0; plane < SMR_NPLANES - 1; ++plane) {
        ...
        if ((nbytes = read(clientfd, r->ra + offset, RX_LEN)) != RX_LEN) {
          perror("read");
          close(clientfd);
          *ret = errno;
          goto err;
...
int rdma_handshake(struct rdma *r) {
  ...
  if (server && pthread_create(&st, NULL, __smr__server_thread, (void *)&sa)) {
  ...
  for (size_t i = 0; i < c->host_id; ++i) {
    ...
    ca[i].id = i;
    ca[i].ret = 0;
    if (pthread_create(ct + i, NULL, __smr__client_thread, (void *)(ca + i))) {
      perror("pthread_create:");
      return -errno;
    }
  }
