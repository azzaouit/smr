...
int __smr__qp_connect(struct rdma *r, int plane, int id) {
    ...
    if (ret) {
        SMR_LOG("Failed to set QP to RTR state");
        return ret;
    }
    ...
    if (ret) SMR_LOG("Failed to set QP to RTS state");
...
void *__smr__client_thread(void *ptr) {
    ...
    for (i = 0; i < MAX_RETRIES; ++i) {
        ...
        if (!connect(sockfd, (struct sockaddr *)&serveraddr,
            ...
            perror("connect");
            SMR_LOG("Connection failed. Retrying...");
        }
    }

    if (i >= MAX_RETRIES) {
        SMR_LOG("Host unreachable.");
        *ret = 1;
        goto exit;
    }
...
int rdma_handshake(struct rdma *r) {
    ...
    for (size_t i = 0; i < c->host_id; ++i) {
        ...
        if (ca[i].ret) {
            SMR_LOG("Client thread exited with nonzero status");
            return ca[i].ret;
        }
    }
