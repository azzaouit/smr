 ...
 * [n log headers ... n slots ... SMR_MAX_BUF slot buffer]
 */
#define SP_HDRS(n) ((struct log_header *)((n)->scratch))
...
int consensus_init(struct config *c, struct consensus *n, size_t log_size) {
    ...
    // Allocate leader election scores
    SMR_ALLOC(n->le.scores, sizeof(int64_t) * npeers);

    // Allocate scratchpad region
    size_t ss = (sizeof(struct log_header) + sizeof(struct slot)) * npeers +
                SMR_MAX_BUF;
    SMR_ALLOC(n->scratch, ss);

    // Init RDMA context
    if ((ret = rdma_init(n->c, &n->r))) {
        SMR_LOG("Failed to init RDMA");
        return ret;
    }

    // Register memory regions
    if ((ret = rdma_add_mr(&n->r, n->log, ls, SMR_REP))) {
        SMR_LOG("Failed to register log");
        return ret;
    }
    if ((ret = rdma_add_mr(&n->r, n->bg, nb, SMR_BG))) {
        SMR_LOG("Failed to register background metadata");
        return ret;
    }
    if ((ret = rdma_add_mr(&n->r, n->scratch, ss, SMR_SCRATCHPAD))) {
        SMR_LOG("Failed to register scratchpad");
        return ret;
    }
...
int __smr__consensus_update_slot(struct consensus *n, size_t index,
    ...
    if (h->size + my_s->len > h->capacity) {
        SMR_LOG("Remote log out of space");
        return -ENOSPC;
    }
    ...
    if ((ret = rdma_write(&n->r, &t, id))) {
        SMR_LOG("Remote slot write failed");
        return ret;
    }
    ...
    t.len = s->len;
    if ((ret = rdma_write(&n->r, &t, id))) {
        SMR_LOG("Remote log  write failed");
        return ret;
    }
    ...
    if ((ret = __smr__consensus_write_log_header(n, id))) {
        SMR_LOG("Failed to write log header");
        return ret;
    }
...
int __smr__log_insert(struct log *l, size_t idx, uint32_t propno, uint8_t *buf,
                      size_t len) {
    if (len > SMR_MAX_BUF) return -EINVAL;
    if (l->h.size + len > l->h.capacity) return -ENOSPC;
    struct slot *s = l->slots + idx;
    s->propno = propno;
    for (size_t i = 0; i < len; ++i) l->h.buf[i] = buf[i];
    s->buf = l->h.buf;
    s->len = len;
    l->h.buf += len;
    l->h.size += len;
    ...
    if (n->fast_path_enabled && n->le.leader == host_id)
        if (!__smr__log_insert(n->log, n->log->h.fuo, ++n->log->h.minprop, buf,
            ...
            for (uint16_t i = 0; i < npeers; ++i)
                if (i != host_id) {
                    int ret = __smr__consensus_update_slot(n, n->log->h.fuo, i);
                    if (!ret)
                        // 3 writes: slot data + log data + log header
                        write_count += 3;
                    else {
                        /* A write failed, abort fast path for this proposal */
                        n->fast_path_enabled = false;
                        SMR_LOG(
                            "Fast path write failed, falling back to slow "
                            "path.");
                        goto slow_path;
                    }
                }
    ...
    while (!done) {
        ...
        for (uint16_t i = 0; i < npeers; ++i)
            if (i != host_id) {
                ...
                if (h->minprop > minprop) minprop = h->minprop;
            }
        minprop += 1;
        ...
        for (size_t i = 0; i < npeers; ++i) {
            struct slot *t =
                (i == host_id) ? n->log->slots + my_fuo : slots + i;
...
int consensus_connect(struct consensus *n) {
    ...
    if (host_id == INITIAL_LEADER) {
        for (uint16_t i = 0; i < npeers; ++i)
            if (i != host_id && __smr__consensus_read_log_header(n, i)) {
                SMR_LOG("Failed to read remote proposal");
                return -errno;
            }
...
// Optimization: Leader Catchup (from Listing 3)
int __smr__leader_catch_up(struct consensus *n) {
    int ret;
    size_t npeers = n->c->n;
    uint16_t host_id = n->c->host_id;

    // Read all follower headers to find the most up-to-date one
    for (uint16_t i = 0; i < npeers; ++i)
        if (i != host_id)
            if ((ret = __smr__consensus_read_log_header(n, i))) {
                SMR_LOG("Failed to read header from %hu during catch-up", i);
                return ret;
            }
    if ((ret = consensus_wait(n, SMR_REP, npeers - 1))) {
        SMR_LOG("Failed to wait for header reads during catch-up");
        return ret;
    }

    // Find the follower with the highest FUO
    struct log_header *hdrs = SP_HDRS(n);
    uint16_t max_fuo_id = host_id;
    uint32_t max_fuo = n->log->h.fuo;
    for (uint16_t i = 0; i < npeers; ++i)
        if (i != host_id && hdrs[i].fuo > max_fuo) {
            max_fuo = hdrs[i].fuo;
            max_fuo_id = i;
        }

    // If we're behind, copy from the most up-to-date follower
    if (max_fuo > n->log->h.fuo) {
        SMR_LOG("Catching up: copying slots [%u, %u) from replica %hu",
                n->log->h.fuo, max_fuo, max_fuo_id);
        for (uint32_t idx = n->log->h.fuo; idx < max_fuo; ++idx) {
            // Read slot from most up-to-date replica
            if ((ret = __smr__consensus_read_slot(n, idx, max_fuo_id))) {
                SMR_LOG("Failed to read slot %u during catch-up", idx);
                return ret;
            }
            // Read the slot data
            if ((ret = __smr__consensus_read_slot_buffer(n, max_fuo_id))) {
                SMR_LOG("Failed to read slot buffer during catch-up");
                return ret;
            }

            if ((ret = consensus_wait(n, SMR_REP, 2))) {
                SMR_LOG("Failed to wait for slot buffer read");
                return ret;
            }

            // Insert into our local log
            struct slot *s = SP_SLOTS(n) + max_fuo_id;
            if ((ret = __smr__log_insert(n->log, idx, s->propno, SP_BUF(n),
                                         s->len))) {
                SMR_LOG("Failed to insert slot during catch-up");
                return ret;
            }
        }
        n->log->h.fuo = max_fuo;
