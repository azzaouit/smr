...
#define SP_CAS(n) ((uint8_t *)n->log + sizeof(struct log) + n->log->h.capacity)
...
int consensus_init(struct config *c, struct consensus *n, size_t log_size) {
    ...
    /*  CAS always writes the original remote value back to the local address.
        Reserve the last SMR_MAX_BUF bytes for CAS operations to avoid
       overwriting the local address.
     */
    ls += SMR_MAX_BUF;
#endif
    SMR_ALLOC(n->log, ls);
...
static inline int __smr__consensus_remote_write(struct consensus *n,
                                                struct mem_tx *t, uint16_t id) {
#if SMR_RDMA_CAS_ENABLED
    struct mem_tx new_tx;
    memcpy(&new_tx, t, sizeof(struct mem_tx));
    /* Convert the RDMA write into an unconditional CAS */
    uint8_t *src = (uint8_t *)t->local_addr;
    static uint8_t zero[SMR_MAX_BUF] = {};
    memset(zero, 0, SMR_MAX_BUF);
    new_tx.local_addr = (uint64_t)SP_CAS(n);
    new_tx.local_plane = SMR_REP;
    new_tx.swap_addr = (uint64_t)src;
    // We can avoid read-then-CAS because we know the remote buffer is zero.
    new_tx.compare_addr = (uint64_t)zero;
    return rdma_cas(&n->r, &new_tx, id);
#else
...
int __smr__consensus_update_slot(struct consensus *n, size_t index,
    ...
    // WRITE 1: metadata for this slot
    struct mem_tx t = {.local_plane = SMR_SCRATCHPAD,
    ...
    if (n->fast_path_enabled)
        if (__smr__log_insert(n->log, n->log->h.fuo, ++n->log->h.minprop, buf,
            ...
            for (uint16_t i = 0; i < npeers; ++i) {
                if (i != host_id) {
                    if (__smr__consensus_update_slot(n, n->log->h.fuo, i) ==
                        0) {
                        write_count +=
                            3;  // 3 writes: slot data + log data + log header
                    } else {
            ...
            if (consensus_wait(n, SMR_REP, write_count) == 0) {
                ...
                n->fast_path_enabled = false;
                SMR_LOG_ERR(
                    "Fast path wait failed, falling back to slow path.");
            }
        }
slow_path:
